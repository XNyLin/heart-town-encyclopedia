...
export function formatPeriodDisplay(cellValue) {
  ...
  if (presetMap[joined]) return presetMap[joined];

  // 跨日組合
  if (joined === "14") return "18:00 - 05:59";
  if (joined === "124") return "18:00 - 11:59";
  if (joined === "134") return "18:00 - 17:59";

  // 其他不連續情況拆開顯示
  return uniquePeriods
    .map((period) => `${ranges[period].start} - ${ranges[period].end}`)
    .join(" / ");
}
